...
public class EncryptUtil {
    ...
    private static String KEY = "LmMGStGtOpF4xNyvYt54EQ==";
    ...
    public static String encrypt(String xmlStr) {
        ...
        try {
            ...
            byte[] key = new byte[8];
            byte[] iv = new byte[8];
            EncryptUtil.getKeyIV(KEY, key, iv);
            SecretKeySpec deskey = new SecretKeySpec(key, "DES");
            IvParameterSpec ivParam = new IvParameterSpec(iv);

            byte[] temp = EncryptUtil.DES_CBC_Encrypt(totalByte, deskey,
                    ivParam);
            result=new Base64().encodeAsString(temp);
    ...
    public static String decrypt(String xmlStr) {
        ...
        try {
            // base64解码
            byte[] encBuf = new Base64().decode(xmlStr);
            // 取密钥和偏转向量
            byte[] key = new byte[8];
            byte[] iv = new byte[8];
            EncryptUtil.getKeyIV(KEY, key, iv);
     ...
     * @throws Exception
     */
    private static byte[] TripleDES_CBC_Encrypt(byte[] sourceBuf,
     ...
     * @throws Exception
     */
    private static byte[] TripleDES_CBC_Decrypt(byte[] sourceBuf,
                                               SecretKeySpec deskey, IvParameterSpec ivParam) throws Exception {
     ...
     * @throws Exception
     */
    private static byte[] DES_CBC_Encrypt(byte[] sourceBuf,
                                         SecretKeySpec deskey, IvParameterSpec ivParam) throws Exception {
        byte[] cipherByte;
        // 使用DES对称加密算法的CBC模式加密
     ...
     * @throws Exception
     */
    private static byte[] DES_CBC_Decrypt(byte[] sourceBuf,
                                         SecretKeySpec deskey, IvParameterSpec ivParam) throws Exception {
    ...
    private static void getKeyIV(String encryptKey, byte[] key, byte[] iv) {
        // 密钥Base64解密
        byte[] buf=new Base64().decode(encryptKey);
        // 前8位为key
        int i;
    ...
    public static String getKey() {
        ...
        for (int i=0;i<16;i++) {
            buff[i]=(byte) (0xFF & random());
        }
        return new Base64().encodeAsString(buff);
    }
    ...
    public static String getMac(String[] var0){
        return DigestUtils.md5Hex(StringUtils.join(var0)).toUpperCase();
    }

    public static String getMac(String[] var,String priate_key){
        byte[] data;
        byte[] p_key;
        String  result=null;
        try{
            data=StringUtils.join(var).getBytes("utf-8");
            p_key=new Base64().decode(priate_key);
            SecretKey secretKey=new SecretKeySpec(p_key,MAC_NAME);
            Mac mac=Mac.getInstance(secretKey.getAlgorithm());
            mac.init(secretKey);
            result=DigestUtils.md5Hex(mac.doFinal(data)).toUpperCase();
        }catch (Exception e){
            e.printStackTrace();
        }
